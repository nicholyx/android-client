...
| GitHub 侧操作：Issue / PR / 提交 / 打包 / 发布 / Roadmap | [maintenance/index.md](maintenance/index.md) —— **含打包验收红线** |
...
1. 读上表对应的 spec 入口（涉及 GitHub 操作时 maintenance/index.md 不可跳过）
2. `bash scripts/lint.sh` 必须先跑一遍确认基线是绿的
3. 完整命令参考与人工验收清单在 `.claude/skills/maintain-loop/SKILL.md`
   （spec 与 skill 是同一套内容的两个视角，改一边必须同步另一边）
...
- [ ] CI 全绿才 squash 合并；Roadmap Issue #25 已同步
- [ ] **红线：自动构建产物不发布 Release；打 tag 只在人工验收通过且用户
      明确指令后**（详见 maintenance/index.md）
